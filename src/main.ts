...
async function getBestMatch(
  ...
  const bucketFiles = await bucket
    ...
    .then(([files]) =>
      files.sort(
        (a, b) =>
          new Date((b.metadata as any).updated).getTime() -
          new Date((a.metadata as any).updated).getTime(),
      ),
    )
    .catch((err) => {
...
async function main() {
  ...
  const storage = inputs.keyFileName 
    ? new Storage({ keyFilename: inputs.keyFileName })
    : new Storage();
  const bucket = storage.bucket(inputs.bucket);
